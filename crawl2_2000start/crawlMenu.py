...
def getItem(itemc):
    ...
    try:
        ...
        try:
            ...
            title="error"
    return (title,des)    
def loadMenuResFromFile(infile,manuListFile):
    ...
    for line in f:
        ...
        if (result != None):
            ...
            print("okay"+line)
    f.close();  
def loadMenuResFromFileJob():
    infile = "chunck/chunck_1_500"
    loadMenuResFromFile(infile,'menu.list')
    
def loadMenuByName(name):
   url = 'https://www.yelp.com/menu/'+name
   result = getMenu(download_content_source(url))
   if (result != None):
        fileName = 'menu/'+name + '.menu'
        fo = open(fileName,'w+',encoding='utf-8');
        fo.write(json.dumps(result))
        print("okay menu "+name)
   else:
